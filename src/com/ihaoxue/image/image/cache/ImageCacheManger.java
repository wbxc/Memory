...
public class ImageCacheManger {
	...
	// 取运行内存阈值的1/8作为图片缓存
	private static final int MEM_CACHE_SIZE = 1024 * 1024 * ((ActivityManager) MemoryApplication.getMemoryApplication()
	...
	public static ImageLoader.ImageListener getImageLinseter2(final BitmapImageEvent mEvBitmapImageEvent,
			final Bitmap defaultImageBitmap, final Bitmap errorImageBitmap){
		...
		return new ImageLoader.ImageListener() {
			...
			public void onResponse(ImageLoader.ImageContainer imageContainer, boolean b) {
				if(imageContainer.getBitmap() != null ){
					mEvBitmapImageEvent.setBitmap( imageContainer.getBitmap()) ;
				}else if(defaultImageBitmap != null ){
					mEvBitmapImageEvent.setBitmap( defaultImageBitmap) ;
				}
			}
			...
			public void onErrorResponse(VolleyError volleyError) {
				if(errorImageBitmap != null){
					mEvBitmapImageEvent.setBitmap( errorImageBitmap) ;
				}
			}
		};
	}
	...
	public static ImageLoader.ImageContainer loadImage2(final String url, BitmapImageEvent mEvBitmapImageEvent,
			final Bitmap defaultImageBitmap, final Bitmap errorImageBitmap){
			return loadImage(url, getImageLinseter2(mEvBitmapImageEvent,defaultImageBitmap, errorImageBitmap));
	}
	
	public interface BitmapImageEvent{
