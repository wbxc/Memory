...
import com.ihaoxue.image.libcore.io.DiskLruCache;
...
class ImageLreCache extends LruCache<String, Bitmap> implements ImageLoader.ImageCache {
    ...
    public ImageLreCache(int maxSize,String diskCacheFodler,int diskCacheSize) {
        ...
        try {
        	mDiskLruCache = DiskLruCache.open(getDiskCacheDir(MemoryApplication.getMemoryApplication(),CACHE_FOLDER_NAME),
					getAppVersion(MemoryApplication.getMemoryApplication()) , 1, DISK_CACHE_SIZE);
		} catch (IOException e) {
			e.printStackTrace();
		}
    }
