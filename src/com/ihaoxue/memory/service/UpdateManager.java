...
import com.ihaoxue.memory.model.UpdateModel;
...
public class UpdateManager {
	...
	public void checkAppUpdate(Context context, final boolean isShowMsg) {
		...
		new Thread() {
			public void run() {
				...
				try {
					// UpdateModel update = ApiClient
					// .checkVersion((AppContext) mContext
					// 网络获取值
					UpdateModel update = new UpdateModel();
					update.setDownloadUrl("http://files.cnblogs.com/luomingui/LCL.Android.apk");
					update.setVersionCode(1);
					update.setUpdateLog("nsnnsn");
					update.setVersionName("好学");
					msg.what = 1;
					msg.obj = update;
				} catch (Exception e) {
					e.printStackTrace();
	...
	private void showNoticeDialog() {
		...
		mCustomMemoryDialog = CustomMemoryDialog.getCustomMemoryDialog(
				mContext, mCustomerInterface);
		mCustomMemoryDialog.setMessage("以后再说", "软件版本更新", "立即更新");
		mCustomMemoryDialog.show();
		/*
		 * AlertDialog.Builder builder = new Builder(mContext);
		 * builder.setTitle("软件版本更新"); builder.setMessage(updateMsg);
	...
	private void showLatestOrFailDialog(int dialogType) {
		...
		if (dialogType == DIALOG_TYPE_FAIL) {
			DemoToast.getInstance(mContext).show("无法获取版本更新信息", 0);
		} else if (dialogType == DIALOG_TYPE_LATEST) {
			DemoImageToast.getInstance(mContext).show(
					R.drawable.setting_last_version);
		}
	...
	private Runnable mdownApkRunnable = new Runnable() {
		...
		public void run() {
			try {
				String apkName = "MemoryApp_" + mUpdate.getVersionName()
						+ ".apk";
				String tmpApk = "MemoryApp_" + mUpdate.getVersionName()
						+ ".tmp";
				// 判断是否挂载了SD卡
				String storageState = Environment.getExternalStorageState();
				if (storageState.equals(Environment.MEDIA_MOUNTED)) {
					savePath = Environment.getExternalStorageDirectory()
							.getAbsolutePath() + "/Memory/Update/";
