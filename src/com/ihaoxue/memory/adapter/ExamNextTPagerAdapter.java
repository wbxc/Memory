...
import com.ihaoxue.memory.ui.ExamNextTMemoryActivity;
import com.ihaoxue.memory.ui.ExamNextTMemoryActivity.MemoryViewHolder;
...
public class ExamNextTPagerAdapter extends PagerAdapter {
	...
	public ExamNextTPagerAdapter(Context mContext, List<View> mWebViews) {
	...
	public void setQuestionArrFlag(int[] questionArrFlag){
		this.questionArrFalg = questionArrFlag ;
	}
	@Override
		...
		if (!arr_loaded[position]) {
			if (questionFlag) {
				if (questionArrFalg[position] == 0) {
					...
					mLinearLayout.addView(DemoSelectLinerLayoutView.getInstance(mContext, memoryViewHolder.questionAnswerNum, mdeDemoSelectEvent)) ;
					...
					// memoryViewHolder.questionContent = TestMemoryActivity.questionAll;
					mWebView.loadUrl(MemoryGlobeManger.DEMO_URL_PATH);
					return childView;
				}else if (questionArrFalg[position] == 1) {
					...
					ImageView mdeDemoImageView = (ImageView)childView.findViewById(R.id.image_item_content) ;
					mdeDemoImageView.setImageBitmap(getBitmap()) ;
					mdeDemoImageView.setOnTouchListener(new OnDoubleClick(mOnTouchEventListener)) ;
					RelativeLayout mRelativeLayout = (RelativeLayout)childView.findViewById(R.id.remmber_again_next) ;
					mRelativeLayout.addView(DemoSelectRemberOrNoView.getInstance(mContext,mdeDemoRemberOrNoRelative )) ;
				}
			}else {
				...
				if (mWeViews.get(position) != null) {
					...
					if (questionArrFalg[position] == 0) {
						...
						if (mLinearLayout != null && mRelativeLayout != null) {
							mRelativeLayout.addView(DemoSelectRelativeView.getInstance(mContext, mDemoSelectRelative));
							mLinearLayout.addView(new DemoSelectLinerLayoutView(mContext, memoryViewHolder.questionAnswerNum,
									mdeDemoSelectEvent));
						}
						mWebView.loadUrl(MemoryGlobeManger.DEMO_URL_PATH);
	...
	private OnClickListener mOnClickListener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			((ExamNextTMemoryActivity) mContext).JumShowImage();
		}
	};
	private DemoRemberOrNoRelative mDemoRemberOrNoRelative = new DemoRemberOrNoRelative() {
		...
		public void unRemmber() {
			// TODO Auto-generated method stub
			((ExamNextTMemoryActivity) mContext).nextQuestion2();
			((ExamNextTMemoryActivity) mContext).runOnUiThread(new Runnable() {
				@Override
				public void run() {
					remmberFunction();
				}
		...
		public void remmber() {
			// TODO Auto-generated method stub
			((ExamNextTMemoryActivity) mContext).nextQuestion();
			((ExamNextTMemoryActivity) mContext).runOnUiThread(new Runnable() {
				@Override
				public void run() {
					remmberFunction();
	...
	private DemoRemberOrNoRelative mdeDemoRemberOrNoRelative = new DemoRemberOrNoRelative() {
		...
		public void unRemmber() {
			...
			((ExamNextTMemoryActivity)mContext).runOnUiThread(new Runnable() {
				...
				public void run() {
					...
					for (int i = 0; i < questionIdArr.length; i++) {
						if (questionArrFalg[i]==0) {
							WebView mWebView = (WebView) mWeViews.get(i).findViewById(R.id.webview_item_content) ;
							MemoryViewHolder memoryViewHolder = (MemoryViewHolder) mWebView.getTag() ;
							RelativeLayout mRelativeLayout = (RelativeLayout) mWeViews.get(i).findViewById(R.id.remmber_again_next) ;
							LinearLayout mLinearLayout = (LinearLayout)mWeViews.get(i).findViewById(R.id.select_question) ;
							if (memoryViewHolder.questionId == questionId) {
	...
	private OnTouchEventListener mOnTouchEventListener = new OnTouchEventListener() {
		...
		public void switchActivity() {
			// TODO Auto-generated method stub
			((ExamNextTMemoryActivity)mContext).switchActivity() ;
			...
			for (int i = 0; i < questionIdArr.length; i++) {
				if (questionArrFalg[i] ==0) {
					...
					if (questionId == memoryViewHolder.questionId) {
						mRelativeLayout.setVisibility(View.GONE) ;
						mLinearLayout.setVisibility(View.VISIBLE) ;
						((ExamNextTMemoryActivity)mContext).remmber_again_button() ;
					}
				}else if(questionArrFalg[i] ==1){
	...
	private DemoSelectEvent mdeDemoSelectEvent = new DemoSelectEvent() {
		...
		public void selectButton(int select) {
			// TODO Auto-generated method stub
			((ExamNextTMemoryActivity)mContext).refreshLoadButton(select) ;
		}
		...
		public void remmberAllKnowleage() {
			// TODO Auto-generated method stub
			for(int i=0 ; i<questionIdArr.length ; i++){
				if (questionArrFalg[i] ==1) {
					
				}else if (questionArrFalg[i] ==0) {
					View mView = mWeViews.get(i) ;
					WebView mWebView = (WebView) mView.findViewById(R.id.webview_item_content) ;
					MemoryViewHolder memoryViewHolder = (MemoryViewHolder) mWebView.getTag() ;
					RelativeLayout mRelativeLayout = (RelativeLayout)mView.findViewById(R.id.remmber_again_next) ;
		...
		public void reloadButton() {
			// TODO Auto-generated method stub
			((ExamNextTMemoryActivity)mContext).reload() ;
		}
	};
	...
	public void refresh(Handler mHandler, final int index, int currentIndex) {
		for (int i = 0; i < questionIdArr.length; i++) {
			if (questionArrFalg[i]==1) {
	...
	public void loadQuestion(Handler mHandler, int index) {
		for (int i = 0; i < questionIdArr.length; i++) {
			if (questionArrFalg[i]==1) {
				...
				final WebView webView = (WebView) mWeViews.get(index).findViewById(
						R.id.webview_item_content);
				final MemoryViewHolder memoryViewHolder = (MemoryViewHolder) webView
						.getTag();
